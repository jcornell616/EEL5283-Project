% 2021.10.17
% modified from ReadJSONnTrignoNSummit.m


function [accmdata, LFPs, NumberofChannels, DeviceSettings]=readAccLFP(pathname, dominant_hand)

%% Get All ACCM Files**********************************
%pathname is session dir, eg 'O:\UH3\SET001\20210416 Month3 Day02\Session1618579044758';
addpath(pathname);%keyboard;
fnames = dir(pathname);
...
for i=1:Nfiles
    ...
    % sensor 9: right hand, sensor 2: right arm ACC
    % sensor 10: left hand, sensor 1: left arm ACC
    % sensor 11: head
   % keyboard; 
   % display(num2str(i))
   if dominant_hand == 1
    accmdata{i,1} = convertDelsysRaw2Array(BehavDataRaw, 11, 13); %right hand, sensor 9
   else
    accmdata{i,1} = convertDelsysRaw2Array(BehavDataRaw, 19, 21); %left  hand, sensor 10
   end;
    % Samplig rate 518
    %extract EMGs
    % sensor 2 ACC (NOT EMG) on right arm
    %emgdata{i,1} = convertDelsysRaw2Array(BehavDataRaw, 9, 9);
    % sensor 1: left arm ACC
...
display(['segmenting', pathname]);
%keyboard;
tvecDiff = diff([0,tvec]);
timethrsh = 0.08; % what unit is it? if it's second it's too short, set to at least 1 sec? hz 2022.03.24
...
%added hz 2022.03.24
%set diff between startIndx to at least 500 - very short trial like reach
%to grasp is about 900
if Nfiles==length(startIndx)
    display([pathname,' success, update trial#']);
    keyboard;
else
    keyboard;
    display(['summit trigno trial mismatch: ',pathname, ' setting trial length threshold']);
    glitchidx = find(diff(startIndx)<500); %trial# glitch+1 - trial# glitch 
    startIndx(glitchidx)=[];
    endIndx(glitchidx)=[];
end
if Nfiles~=length(startIndx)
    display(['summit trigno trial mismatch: ',pathname]);
    figure; plot(normalize(XX(:,1))); hold on;
stem(startIndx,ones(1,length(startIndx)).*3 ); hold off;
    keyboard; 
    % this is where we manually edit the data sheet to throw out trials that we cannot segment correctly
    % if everything is correct, "summit" column matches "trigno" column.
    % But if they don't match we have to manually specify
end
